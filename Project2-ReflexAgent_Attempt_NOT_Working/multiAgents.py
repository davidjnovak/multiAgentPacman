...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        goalIndex = 0
        for x in range(len(successors)):
            actionValue = self.value(successors[x], 1, 0,self.alpha,self.beta)
            if actionValue > maxValue:
                maxValue = actionValue
                goalIndex = x
    ...
    def MINvalue(self, gameState, agentIndex, depthSoFar, alpha, beta):
        ...
        for move in legal:
            successor = gameState.generateSuccessor(agentIndex,move)
            if agentIndex + 1 == gameState.getNumAgents():#pacmans turn is next
                x = min(x,self.value(successor, 0, depthSoFar + 1, alpha, beta))
            else: #another ghost turn
    ...
    def getAction(self, gameState):
        """
          ...
          gameState.getNumAgents():
            Returns the total number of agents in the game
        """
        print("Expectimax with depth ", self.depth)
        legal = gameState.getLegalActions(0)
        successors = [gameState.generateSuccessor(0, action) for action in legal]
        maxValue = -float('inf')
        goalIndex = 0
